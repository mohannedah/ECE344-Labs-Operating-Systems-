                                   ...
                                   void *page_table)
{
    ...
    printf("Fatal page fault!\n"
           "  Virtual address: 0x%" PRIX64 "\n"
           "  Page table: 0x%" PRIX64 "\n"
           "  Level: %d\n"
           "  PTE:\n"
           "    PPN: 0x%" PRIX64 "\n"
           "    Flags: %s%s%s%s\n",
...
static void *translate_address(void *virtual_address, uint64_t *entry)
{
    ...
    physical_address = (void *)(((uint64_t)physical_address) | (((uint64_t)virtual_address) & 0xFFF));
...
void vms_write(void *virtual_address, int value)
{
    ...
    int *pointer = (int *)translate_address(virtual_address, entry);
    *pointer = value;
}
